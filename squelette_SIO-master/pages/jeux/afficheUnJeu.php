<?php
	...
	$SQL = "SELECT commentaire FROM avis WHERE idJeu = ? "; // ecrire la requete qui va chercher les commentaires du jeu affiché
	$stmt = $connexion->prepare($SQL);
	$stmt->execute(array($idJeu)); // on passe dans le tableaux les paramètres si il y en a à fournir (ici l'identifiant du jeu)
	...
	$SQL = "SELECT avis.commentaire,avis.date,membre.nomMembre FROM avis JOIN membre WHERE avis.idMembre = membre.idMembre ";
	$stmt = $connexion->prepare($SQL);
	$stmt->execute(array($idJeu)); // on passe dans le tableaux les paramètres si il y en a à fournir (ici l'identifiant du jeu)
	$membre = $stmt->fetch(PDO::FETCH_ASSOC); // on met le resultat de la requete dans un tableau
	var_dump($jeu); // on affiche le contenu de la variable $jeu (ici un tableau php array())
	$stmt->closeCursor(); // on ferme le curseur des résultats*/
	...
	<tbody> 
		<tr>
			<td rowspan="2" class="photo">
				...
				<?php if ($jeu['photo'] == false){
					echo ("aucune image n'est disponible pour ce jeu");
				}
				?>
